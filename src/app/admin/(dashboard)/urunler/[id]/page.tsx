  ...
  useEffect(() => {
    params.then(({ id: resolvedId }) => {
      ...
      Promise.all([
        ...
        fetch(`/api/products/${resolvedId}`).then((r) => r.json()),
        ...
        setImageUrls(
          (product.images || [])
            .filter((img: { url: string }) => img.url !== '/images/products/placeholder.svg')
            .map((img: { url: string }) => img.url)
        );
        setSpecs(
          product.specifications?.length
            ? product.specifications.map((s: { key: string; value: string }) => ({ key: s.key, value: s.value }))
            : [{ key: '', value: '' }]
        );
        setFetching(false);
      });
  ...
  return (
    <div>
      <div className="flex items-center gap-3 mb-6">
        <Link href="/admin/urunler" className="text-[#6B7280] hover:text-[#1A1A2E] transition-colors">
          <FiArrowLeft className="w-5 h-5" />
        </Link>
        <h1 className="text-2xl font-bold text-[#1A1A2E]">Ürünü Düzenle</h1>
      </div>
      ...
      <form onSubmit={handleSubmit} className="max-w-2xl space-y-6">
        ...
        <div className="flex gap-3">
          <Link
            href="/admin/urunler"
            className="flex-1 text-center border border-gray-200 hover:bg-gray-50 text-[#6B7280] font-medium py-3 rounded-lg transition-colors text-sm"
          >
            İptal
          </Link>
          <button
            type="submit" disabled={loading}
            className="flex-1 bg-[#0057A8] hover:bg-[#004080] disabled:opacity-50 text-white font-medium py-3 rounded-lg transition-colors"
          >
